...
attributes.
*/
function createColorBox(colors, size) {
...
function renderPaletteRow(colors, parent) {
  ...
  colors.forEach(color => {
    const colorBox = createColorBox(color, getElementWidth(parent));
...
function renderPaletteTable() {
  ...
  for (const palettes in COLORS_9) {
    const palette = COLORS_9[palettes];
    renderPaletteRow(palette, main);
  }
}
renderPaletteTable();
